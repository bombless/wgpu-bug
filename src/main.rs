...
impl Action for State {
    fn update(&mut self) {
        self.app.queue.write_buffer(self.color.buffer(0, 0), 0, bytemuck::cast_slice(&[1f32, 0.0, 0.0, 1.0]));
    }
    fn new(app: AppSurface) -> Self {
        ...
        let color = uniform::Proxy::new(bytemuck::cast_slice(&[1.0f32; 4]), &[1], &app.device, ShaderStages::FRAGMENT);

        let render_pipeline_layout =
            app.device
                .create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                    label: Some("Render Pipeline Layout"),
                    bind_group_layouts: &[color.layout()],
    ...
    fn render(&mut self) -> Result<(), wgpu::SurfaceError> {
...

        {
            ...
            render_pass.set_bind_group(0, self.color.bind_group(0, 0), &[]);
